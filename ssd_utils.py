...
class BBoxUtility(object):
    ...
    def assign_boxes(self, boxes):
        ...
        encoded_boxes = encoded_boxes.reshape(-1, self.num_priors, 5)    # encoded_boxes=[obj_num, prior_num, 5]
        best_iou = encoded_boxes[:, :, -1].max(axis=0)    # choice max IOU's value for every prior boxes
        best_iou_idx = encoded_boxes[:, :, -1].argmax(axis=0)    # max IOU corresponding objects idx
        best_iou_mask = best_iou > 0                      # max IOU's box mask( or idx)
